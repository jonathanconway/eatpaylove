  ...
  describe('constructor', () => {
    it('migrates all legacy "string-date" indeces over to the new dateIndex',
      ...
      const presenter = new AppPresenter(
        model,
        newModel => Object.assign(model, newModel))

      expect(model.dates['2017-09-01']).not.toBeDefined()
      expect(model.dates['2017-09-05']).not.toBeDefined()
      expect(model.dates['2017-10-05']).not.toBeDefined()
      expect(model.dates[dateIndex(new Date(2017, 8, 1))])
        .toBeDefined()
      expect(model.dates[dateIndex(new Date(2017, 8, 5))])
        .toBeDefined()
      expect(model.dates[dateIndex(new Date(2017, 9, 5))])
        .toBeDefined()
      expect(model.dates[dateIndex(new Date(2017, 8, 1))])
        .toEqual({ items: [1,2] })
      expect(model.dates[dateIndex(new Date(2017, 8, 5))])
    ...
    describe(`merely removes items at legacy "string-date", without affecting
      ...
      it(`if there aren't any items at legacy "string-date"`, () => {
        let model = new AppModel(new Date(),
          {
            ['2017-09-01']: { },
            [dateIndex(new Date(2017, 8, 1))]: { items: [3,4] }
          })
    ...
    it(`if there is no date at new dateIndex`, () => {
      ...
      expect(model.dates[dateIndex(new Date(2017, 8, 2))])
        .toEqual({ items: [3,4] })
      expect(model.dates['2017-09-01'])
        .not.toBeDefined()
    })
  })
